...
function statusLabel(s) {
  const map = { DR: 'Draft', IP: 'In Progress', CO: 'Booked', TE: 'Booked', CL: 'Closed', VO: 'Voided' }
...
async function loadLookups() {
  const [w, pt, pm, u, org, pl, tx] = await Promise.allSettled([
...
async function selectVendor(v) {
  ...
  // 1. Coba ambil dari object list (v) dengan urutan: khusus PO -> lalu general
  const ptId = extractId(v.pOPaymentTerms) || extractId(v.paymentTerms)
  const pmId = extractId(v.pOPaymentMethod) || extractId(v.paymentMethod)
  const plId = extractId(v.purchasePricelist) || extractId(v.priceList)

  if (ptId) form.value.paymentTerms  = ptId
  if (pmId) form.value.paymentMethod = pmId
  if (plId) form.value.priceList     = plId

  // 2. Jika masih ada yang kosong, fetch detail penuh vendor sebagai pelengkap
...
async function doProcessOrder() {
  ...
  try {
    ...
    showToast('Purchase Order berhasil di Process (In Progress).')
...
async function doBookOrder() {
  ...
  try {
    ...
    try {
      ...
      showToast('Purchase Order berhasil di Book dan Payment Plan terbentuk.')
    } catch (planErr) {
      console.error('[doBookOrder] Payment plan gagal:', planErr.message)
      showToast('Purchase Order di Book, tapi Payment Plan gagal: ' + planErr.message, 'warning')
    }
...
async function doVoidOrder() {
  ...
  try {
    ...
    showToast('Purchase Order di Void.')
    await updateViewState(updated)
  } catch (e) {
    actionError.value = e?.response?.data?.response?.error?.message || e.message
  } finally { actionLoading.value = '' }
...
async function doReactivateOrder() {
  ...
  try {
    ...
    showToast('Purchase Order di Reactivate kembali ke Draft.')
    await updateViewState(updated)
    viewPaymentSchedule.value = []
  } catch (e) {
    actionError.value = e?.response?.data?.response?.error?.message || e.message
...
async function doCloseOrder() {
  ...
  try {
    ...
    showToast('Purchase Order berhasil di-Close.')
