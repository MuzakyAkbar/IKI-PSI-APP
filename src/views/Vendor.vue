      ...
      <div class="content-card">
        ...
        <div v-if="activeTab==='list'">
          ...
          <div class="pagination">
            ...
            <button v-for="(p, index) in paginationPages" :key="index" :class="['page-btn', p===currentPage?'page-btn--active':'']" @click="typeof p==='number'&&goPage(p)">{{ p }}</button>
...
async function load() {
  ...
  try {
    const res = await fetchVendors({ startRow: (currentPage.value-1)*pageSize, pageSize, searchKey: searchQuery.value, sortCol: sortCol.value, sortDir: sortDir.value })
