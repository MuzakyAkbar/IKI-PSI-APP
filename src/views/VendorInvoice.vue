...
// ── computed labels (auto-fill dari vendor)
...
async function loadInvoices() {
  ...
  try {
    const data = await fetchAllInvoices({ 
      ...
      sortCol: sortCol.value, // <--- Kirim kolom yang diurutkan
      sortDir: sortDir.value  // <--- Kirim arah urutannya
    })
...
function selectVendor(v) {
  ...
  // Vendor PO fields: pOPaymentTerms, pOPaymentMethod, purchasePricelist
  ...
  if (!ptId || !pmId || !plId) {
    ...
    }).catch(e => console.warn('[selectVendor] fallback failed:', e))
  }
...
async function doPostAccounting() {
  ...
  accountingFacts.value = []; accountingError.value = ''; viewTab.value = 'accounting'
  accountingLoading.value = true
  try {
  ...
  } finally { accountingLoading.value = false; posting.value = false }
}
...
async function doUnpostAccounting() {
  ...
  try {
    ...
    accountingError.value = e2?.response?.data?.response?.error?.message || e2.message || 'Failed to load accounting data'
  } finally { accountingLoading.value = false; unposting.value = false }
}
