            ...
            <div class="filter-group">
              ...
              <div class="acc-wrap">
                ...
                <ul v-if="showVendorDrop && vendorOptions.length" class="acc-dropdown">
                  <li class="acc-opt acc-opt--all" @mousedown.prevent="selectAllVendors">
                    <span class="check-icon" :class="{ 'check-icon--on': selectedVendors.length === 0 }">✓</span>
                    Semua Vendor
                  </li>
                  <li
                    ...
                    @mousedown.prevent="toggleVendor(v)"
                  >
                    <span class="check-icon" :class="{ 'check-icon--on': isSelected(v.id) }">✓</span>
                    {{ v.name }}
                  </li>
                </ul>
              </div>
              <div v-if="selectedVendors.length" class="chips-wrap">
                <span v-for="v in selectedVendors" :key="v.id" class="chip">
                  {{ v.name }}
                  <button class="chip-rm" @click="removeVendor(v.id)">×</button>
                </span>
              </div>
            </div>
            <div class="filter-group">
...
async function onVendorSearch() {
  ...
  vendorTimer = setTimeout(async () => {
    ...
    // Vendor = businessPartnerCategory name 'Vendor' OR salesTransaction=false related
    ...
    vendorOptions.value = res.data?.response?.data ?? []
  }, 300)
}
function onVendorBlur() { setTimeout(() => { showVendorDrop.value = false }, 200) }
function isSelected(id) { return selectedVendors.value.some(v => v.id === id) }
function toggleVendor(v) {
  const idx = selectedVendors.value.findIndex(x => x.id === v.id)
  if (idx >= 0) selectedVendors.value.splice(idx, 1)
  else selectedVendors.value.push({ id: v.id, name: v.name })
}
function selectAllVendors() { selectedVendors.value = [] }
function removeVendor(id) { selectedVendors.value = selectedVendors.value.filter(v => v.id !== id) }
...
// ── main report — salesTransaction=false for AP
async function runReport() {
  running.value = true; reportError.value = ''; summaryRows.value = []
  try {
    const asOf = filters.value.asOfDate

    // KEY DIFFERENCE: salesTransaction=false → AP (Payables)
    let where = `e.salesTransaction = false and e.documentStatus = 'CO' and e.outstandingAmount > 0`
    where += ` and e.invoiceDate <= '${asOf}'`
    if (selectedVendors.value.length > 0) {
      const ids = selectedVendors.value.map(v => `'${v.id}'`).join(',')
      where += ` and e.businessPartner.id in (${ids})`
    }

    // Fetch all outstanding AP invoices with pagination
    let allInvoices = []
    let startRow = 0
    const pageSize = 200
    while (true) {
...
async function exportExcel() {
  try {
    ...
    const ws = wb.addWorksheet('AP_AgingBalance')
    const g = filters.value
    ...
    ws.addRow(['Payables Aging Schedule'])
    ...
    for (const row of summaryRows.value) {
      ws.addRow([row.bpName, row.current, row.g1, row.g2, row.g3, row.g4, row.g5, row.total, row.credits, row.net])
    }

    const gt = grandTotal.value
    const totRow = ws.addRow(['TOTAL:', gt.current, gt.g1, gt.g2, gt.g3, gt.g4, gt.g5, gt.total, gt.credits, gt.net])
    ...
    a.download = `AP_Aging_${g.asOfDate}.xlsx`; a.click()
...
async function exportPdf() {
  try {
    ...
    doc.text('Payables Aging Schedule', pageW / 2, 40, { align: 'center' })
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`Generated on ${formatDisplayDate(new Date().toISOString().slice(0,10))}`, 40, doc.internal.pageSize.getHeight() - 20)
      doc.text(`Page ${i} of ${pageCount}`, pageW - 40, doc.internal.pageSize.getHeight() - 20, { align: 'right' })
    }
    doc.save(`AP_Aging_${g.asOfDate}.pdf`)
...
async function exportDetailExcel() {
  try {
    ...
    const ExcelJS = window.ExcelJS
    const wb = new ExcelJS.Workbook()
    const ws = wb.addWorksheet('AP_AgingDetail')
    const g = filters.value
    const row = detailRow.value

    ws.addRow(['Payables Aging Schedule Details'])
    ...
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url
    a.download = `AP_Aging_Detail_${row.bpName}_${g.asOfDate}.xlsx`; a.click()
    URL.revokeObjectURL(url)
...
async function exportDetailPdf() {
  try {
    ...
    doc.text('Payables Aging Schedule Details', pageW / 2, 36, { align: 'center' })
    ...
    doc.save(`AP_Aging_Detail_${row.bpName}_${g.asOfDate}.pdf`)
...
onMounted(() => {
  // Pre-load vendor list
  api.get('/org.openbravo.service.json.jsonrest/BusinessPartner', {
    params: { _where: `e.vendor = true`, _startRow: 0, _endRow: 100 },
  }).then(res => { vendorOptions.value = res.data?.response?.data ?? [] }).catch(() => {})
})
</script>
...
.aging-table thead .aging-header-row th { background: #3a3a3a; color: #fff; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: .05em; padding: 10px 12px; border: none; white-space: nowrap; }
...
.detail-table thead th { background: #3a3a3a; color: #fff; font-size: 10.5px; font-weight: 700; padding: 8px 10px; white-space: nowrap; }
