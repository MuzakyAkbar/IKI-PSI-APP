          ...
          <div class="modal-body">
            <div class="detail-grid">
              ...
              <div class="detail-item" style="grid-column:1/-1">
                ...
                <div class="detail-value">{{ viewModal.description || '—' }}</div>
              </div>
              <div class="detail-item">
            ...
            <div class="sub-section">
              <div class="sub-title">Storage Bin</div>
              <table class="table table--sub">
                <thead><tr>
                  <th>Storage Bin Code</th><th>Storage Bin Name</th>
                  <th>Corridor(X)</th><th>Shelves(Y)</th><th>Height(Z)</th>
                </tr></thead>
                <tbody>
                  <tr v-if="subLoading"><td colspan="5" class="td-empty"><div class="loading-dots"><span></span><span></span><span></span></div></td></tr>
                  <tr v-else-if="subBins.length===0"><td colspan="5" class="td-empty">—</td></tr>
                  <tr v-else v-for="b in subBins" :key="b.id">
                    <td><span class="code-badge">{{ b.searchKey }}</span></td>
                    <td>{{ b.searchKey }}</td>
                    <td>{{ b.x || '—' }}</td>
                    <td>{{ b.y || '—' }}</td>
                    <td>{{ b.z || '—' }}</td>
...
async function load() {
  ...
  try {
    const res = await fetchWarehousesPage({ startRow, pageSize, searchKey: searchQuery.value })
...
async function openViewModal(r) {
  openDropdown.value = null; viewModal.value = r; _viewRow.value = r
  subBins.value = []; subLoading.value = true
  try {
    const res = await fetchStorageBinsPage({ startRow: 0, pageSize: 50, warehouseId: r.id })
    subBins.value = Array.isArray(res.data) ? res.data : (Array.isArray(res) ? res : [])
  } catch (e) { console.warn('subBins failed', e) }
...
async function doDelete() {
  ...
  try {
    await deleteWarehouse(deleteModal.row.id)
    deleteModal.show = false
    showToast(deleteModal.row.active ? 'Warehouse deactivated.' : 'Warehouse activated.')
    load()
