            ...
            <tbody>
              ...
              <template v-else>
                <tr v-for="r in rows" :key="r.id" class="tr-data">
                  ...
                  <td class="td-clip td-name">{{ r['organization$_identifier'] || '—' }}</td>
        ...
        <div class="modal modal--xl">
          ...
          <div class="modal-body">
            ...
            <div v-if="activeFormTab==='transaction'">
              <div class="form-grid-3">
                ...
                <div class="form-group">
                  ...
                  <input v-model="form.organization" class="form-input" placeholder="Organization" disabled />
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <input v-model="form.documentNo" class="form-input" placeholder="Doc no." disabled />
                </div>
                <div class="form-group">
              ...
              <div class="form-grid-3">
                ...
                <div class="form-group">
                  ...
                  <input v-model="form.invoiceAddress" class="form-input" placeholder="Invoice Address" />
                </div>
                <div class="form-group">
                ...
                <table class="table table--lines">
                  ...
                  <tbody>
                    ...
                    <tr v-for="(line, idx) in lines" :key="idx">
                      ...
                      <td><input v-model.number="line.orderedQuantity" type="number" min="0" class="form-input form-input--sm" @input="calcLine(line)" /></td>
                      <td>
                        <div class="acc-wrap">
            ...
            <div v-if="activeFormTab==='payment'">
              <div class="toolbar" style="padding:0 0 14px; border-bottom:1px solid var(--border); margin-bottom:14px;">
                <div></div>
                <button class="btn btn--primary btn--sm" @click="addPaymentLine">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 5v14M5 12h14"/></svg>
                  Add Payment
                </button>
              </div>
              <div class="table-wrap" style="margin-bottom:0">
                <table class="table">
                  ...
                  <tbody>
                    <tr v-if="paymentLines.length===0"><td colspan="4" class="td-empty" style="padding:20px">No payment terms. Click "Add Payment".</td></tr>
                    <tr v-for="(pl, idx) in paymentLines" :key="idx">
                      <td><input v-model.number="pl.percentage" type="number" min="0" max="100" class="form-input form-input--sm" placeholder="%" /></td>
                      <td><input v-model.number="pl.price" type="number" min="0" class="form-input form-input--sm" placeholder="Price" /></td>
                      <td><input v-model="pl.note" class="form-input form-input--sm" placeholder="Note" /></td>
                      <td>
                        <button class="btn-rm-line" @click="paymentLines.splice(idx,1)">
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18M6 6l12 12"/></svg>
                        </button>
                      </td>
                    </tr>
      ...
      <div v-if="showViewModal" class="modal-overlay" @mousedown.self="showViewModal=false">
        <div class="modal modal--xl">
          ...
          <div class="modal-body" v-if="viewRow">
            ...
            <!-- View Order Lines -->
            <div class="section-divider" style="margin-top:20px">Order Lines</div>
            ...
            <div v-else class="table-wrap" style="margin-bottom:0">
              <table class="table table--lines">
                <thead><tr>
                  <th>#</th><th>Product</th><th>Qty</th><th>UOM</th><th>Unit Price</th><th>Net Amount</th>
                </tr></thead>
                <tbody>
                  <tr v-if="viewLines.length===0"><td colspan="6" class="td-empty">No lines.</td></tr>
...
function statusLabel(s) {
  const map = { DR: 'Draft', TE: 'Booked', CO: 'Completed', CL: 'Closed', VO: 'Voided' }
  return map[s] || s || 'Unknown'
}
function statusClass(s) {
  if (s === 'DR') return 'status-pill--draft'
  if (s === 'TE' || s === 'BO') return 'status-pill--portal'
...
async function loadLookups() {
  const [w, d, pt, pm, u] = await Promise.allSettled([
    fetchWarehouses(), fetchDocumentSequences(), fetchPaymentTerms(), fetchPaymentMethods(), fetchUOMs(),
  ])
...
async function onUomSearch(line) {
  ...
  uomTimers.set(line, setTimeout(async () => {
    ...
    if (!q) {
      line.uomOptions = uoms.value.slice(0, 20)
    } else {
...
function addPaymentLine() { paymentLines.value.push({ percentage: '', price: '', note: '' }) }
    ...
    if (orderId) {
      ...
      for (const line of lines.value) {
        ...
        if (line.id) {
          ...
          await createOrderLine(orderId, linePayload)
        }
      }
    }
...
.status-pill--portal { background: #fff7ed; color: #c2410c; }
