...
api.interceptors.response.use(
  ...
  (err) => {
    console.error('[PurchaseOrder HTTP error]', err.response?.status, JSON.stringify(err.response?.data))
...
export const PURCHASE_ORDER_DOCTYPE_ID = '53F55A814CA045AE9561B7E247FF9569'
...
function buildOrderPayload(data) {
  ...
  return {
    ...
    documentType:        PURCHASE_ORDER_DOCTYPE_ID,
...
export async function createOrderPaymentPlan(orderId, orderData) {
  ...
  if (!pmId) {
    throw new Error('Payment Method tidak ditemukan untuk order ini. Pastikan vendor memiliki Payment Method yang valid.')
  }
...
export async function fetchVendors(search = '') {
  ...
  const res = await api.get('/org.openbravo.service.json.jsonrest/BusinessPartner', {
    params: {
      ...
      _endRow:   50,
      _orderBy:  'e.name asc',
      _selectedProperties: 'id,name,searchKey,vendor,active,pOPaymentTerms,pOPaymentTerms$_identifier,pOPaymentMethod,pOPaymentMethod$_identifier,purchasePricelist,purchasePricelist$_identifier,paymentTerms,paymentMethod,priceList',
    },
  })
  return res.data?.response?.data ?? []
}
...
export async function fetchPriceLists() {
  const res = await api.get('/org.openbravo.service.json.jsonrest/PricingPriceList', {
    params: {
      _where:    `e.active = true and e.salesPriceList = false`,
      _startRow: 0,
      _endRow:   100,
      _orderBy:  'e.name asc',
    },
